...
- **RD storage links never reach the browser.** Heat maps are served through `GET /api/scans/{requestId}/heatmap`, which re-reads a fresh link each time; the explanation page opens through its redirect route.
