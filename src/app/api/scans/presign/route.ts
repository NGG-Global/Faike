 ...
 * file straight to that URL, so media never passes through Faike's
 * functions, then polls GET /api/scans/{requestId}.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return json<PresignResponse>({ requestId: upload.requestId, uploadUrl: upload.response.signedUrl });
  } catch (error) {
